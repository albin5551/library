...
public interface BookRepository extends Repository<Book, Integer> {
    ...
    // @Query(value = "select * from book s where s.book_name like %:keyword% ",
    // nativeQuery = true)
    // List<Book> findByKeyword(@Param("keyword") String keyword);

    // @Query(value = "update book set stock=stock-1 where book_id=?1", nativeQuery
    // = true)
    // public void updateStock(Integer bookId);

    // @Query(value = "SELECT * FROM book where stock !=0 AND status = 1 AND
    // book_name like %?1%",nativeQuery = true)
    ...
    // Book findBybookId(Integer bookId);
    // Page<Book>findByCategoryCategoryId(List< Integer> categoryId,Pageable
    // pageable);
